...
//        }
        ...
        $Caps = empty($Caps) ? 0 : $Caps;
        $additional_repairs = empty($additional_repairs) ? 0 : $additional_repairs;
        echo $query = "insert into JobDetailsTable(JobID,Room,Floor,Top,Bottom,RegGlass,DoublePaneRegular,LargeGlass,DoublePaneLarge,"
                . "RedTip,UltraLift,BlackTip,BlockTackle,LAMI,Plexi,RoughWire,RoughWireClear,PolyWire,PolyWireClear,Height,Width,InsulatedUnit,"
                . "NewScreen,ScreenRepair,Moldings,WindowGuards,Capping,Locks,Shoes,TiltLatch,Pivot,Caps,Notes,RedTip_c,UltraLift_c,BlackTip_c,BlockTackle_c,additional,GTQty,Status) values "
                . "('" . $JobID . "','" . $Room . "','" . $Floor . "','" . $Top . "','" . $Bottom . "','" . $RegGlass . "','" . $DoublePaneRegular . "','" . $LargeGlass . "',"
                . "'" . $DoublePaneLarge . "','" . $RegRepairs . "','" . $UltraLift . "','" . $BlackTip . "',"
...
//    $pdf->writeHTML($shortcodes, true, false, false, false, '');
    ...
    if (!empty($file)) {
        $html .= '<tr>';
        $html .= '<td style="width: 38%; height: 100px; text-align: center;">';
        $html .= '<br><br><div style="width: 100px; border-bottom: 1px solid black;">';
        ...
        $html .= '<tr><td style="width: 38%;"></td><td style="width: 10%;"></td>';
    }
    if (!empty($techfile)) {
        $html .= '<td style="width: 38%; height: 100px; text-align: center;">';
        ...
        $html .= '</div><br>';
        $html .= '<strong style="text-align: left;">Tech Signature</strong>'; // Add label here
        $html .= '</td></tr>';
    } else {
        $html .= '<td style="width: 38%;"></td></tr>';
    }
...
//$initialY = $pdf->GetY();  // Get the initial Y position for placing images
//$imageHeight = 20;         // Desired height for both images
//$imageWidth = 60;          // Desired width for both images
//$gapBetweenImages = 10;    // Desired gap between the two images
//
//// Set X positions for images to be on the same line
//$tenantImageX = 15; // X position for the tenant image
//$techImageX = $tenantImageX + $imageWidth + $gapBetweenImages; // Position tech image to the right of the tenant image
//
//// Ensure that both images are aligned to the same initial Y position
//if (!empty($file)) {
//    // Place tenant image
//    $pdf->Image($file, $tenantImageX, $initialY, $imageWidth, $imageHeight);
//    // Draw the underline for tenant image
//    $pdf->Line($tenantImageX, $initialY + $imageHeight + 1, $tenantImageX + $imageWidth, $initialY + $imageHeight + 1);
//}
//
//if (!empty($techfile)) {
//    // Place tech image
//    $pdf->Image($techfile, $techImageX, $initialY, $imageWidth, $imageHeight);
//    // Draw the underline for tech image
//    $pdf->Line($techImageX, $initialY + $imageHeight + 1, $techImageX + $imageWidth, $initialY + $imageHeight + 1);
//}
//
//// Adjust the Y position after images
//$pdf->SetY($initialY + $imageHeight + 3); // Move Y position down slightly to avoid overlap with subsequent content
...
//--------------------Size and Qty is missing
            ...
            if (additionalBalanceFilled && addMoreValiR_Empty && addMoreValiL_Empty) {
                alert_msg += 'Fill the glass section input value\n';
                isValid = false;
            }
...
//    for add repair balances
            ...
            if (addrepBFilled && addrepVali_Empty) {
                alert_msg += 'Fill the additional input value\n';
                isValid = false;
            }
//for sash repair section
            ...
            if (sashFilled && addMoreValisash_t_b_Empty) {
                alert_msg += 'Fill the sash repair section input value\n';
                isValid = false;
            }
...
//                    isValid = false;
////            alert("Please enter a value greater than 0 in the glass section.");
//                    $(this).focus();
//                }
//            });
//            
    ...
    $('#main_div > div').last().find('.forzero').each(function () {
        if (Number($(this).val()) === 0) {
            hasZero = true;
            $(this).focus();  // Focus on the first .zero input found with a 0 value
            return false;  // Exit the loop after the first 0 is found
        }
    });
    if (hasZero) {
         alert_msg += 'Please enter a value greater than 0 in the glass section.\n';
...
//  validation check for the "Add +" button
        ...
        $('.gls_tp_divvv').last().show(); // This ensures the appended content is visible
    });
