...
//if ($crow->status == 1) {
//  updateCron($conn, 2);
$csvDir = '/home/bitnami/htdocs/csvfile_test/';
    ...
    switch ($tblName) {
        ...
        case "WCreditDetails":
            ...
            if (($handle = fopen($file, "r")) !== FALSE) {
                ...
                if (count($row_arr) > 0) {
                    for ($i = 0; $i <= count($row_arr); $i += 1) {
                        $new_arr = array_slice($row_arr, $i, 1);
                        $row_str = implode(",", $new_arr);
                        try {
                    ...
                    for ($i = 0; $i <= count($row_arr); $i += 1) {
                        ...
                        try {
                            ...
                            try {
                                ...
                                echo $message = "Data error/special characters in line# " . $i;
                                $log_qry = "insert into InsertErrorLogs(table_name,message) values('" . $tblName . "','" . $message . "')";
                                $conn->query($log_qry);
                                $mail = new PHPMailer(true);
                                send_mail($mail, $tblName, $i, FROM_EMAIL_ADDRESS, SMTP_USERNAME, SMTP_PASSWORD, FROM_NAME);
...
function send_mail($mail, $tblName, $i, $from_email_address, $smtp_username, $smtp_password, $from_name) {
    ...
    try {
        ...
        $mail->addAddress('pandatsachin38@gmail.com');
        //Content
